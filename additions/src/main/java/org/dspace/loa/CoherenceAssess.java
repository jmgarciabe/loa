...
public class CoherenceAssess {
	...
	public static double perform(DSpaceObject dso) throws Exception {
		...
		if (item.getMetadata("dc.type") != null && item.getMetadata("dc.format.mimetype") != null) {
			String path = CoherenceAssess.class.getResource("/allowedValues/media-types.csv").toString();
	...
	private static void init() throws LangDetectException {
		...
		URL url = CoherenceAssess.class.getResource("profiles");
		...
		try {
			...
			while (entries.hasMoreElements()) {
				...
				if (entry.contains("profiles/") && !entry.endsWith("profiles/")) {
					try (InputStream in = CoherenceAssess.class.getClassLoader().getResourceAsStream(entry);) {
